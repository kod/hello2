...
export function* bannerHomeRecommendFetchWatchHandle() {
  try {
    const response = [];
    ...
    yield put(bannerHomeRecommendFetchSuccess(response));
  } catch (err) {
...
export function* bannerHomeRecommendFetchWatch() {
  yield takeEvery(
    BANNER_HOME_RECOMMEND.REQUEST,
    bannerHomeRecommendFetchWatchHandle,
  );
}
